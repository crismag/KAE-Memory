...
- `src/kae_memory/api/` — the HTTP contract (ADR-0014): projects, sessions,
  messages, knowledge, runs, readiness, blockers, contradictions, and `GET
  /health`. Served by `python -m kae_memory.api`.
...
- `tests/` — 159 tests including the HTTP contract and the cross-run persistence proof, the
...
The Review agent, Server-Sent Events, the generated client, and the user
interface are **not** implemented. Check
...
`make check` passes: ruff, ruff format, mypy strict, and 159 tests against
