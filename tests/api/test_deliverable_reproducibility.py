...
class TestTheMigrationDoesNotBackfill:
    def test_the_column_defaults_to_ineligible(self, factory: sessionmaker[Session]) -> None:
        """Defaulting to true would have made every legacy row claim provability."""

        with factory() as session:
            default = session.execute(
                text(
                    "SELECT column_default FROM information_schema.columns "
                    "WHERE table_name = 'deliverables' AND column_name = 'publication_eligible'"
                )
            ).scalar()
